...
// 423095203
// Practica 7: Texturizado (Modelo de dado con textura y cubo)
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Practica 7: Texturizado (Dado) - Eric Ramirez", nullptr, nullptr);
	...
	glBindVertexArray(0);// incializamos el VAO a 0 para evitar que se modifique accidentalmente

	// Load textures
	GLuint texture1; //genermaos un identificador
	...
	stbi_set_flip_vertically_on_load(true);// configuramos la carga de la imagen para que se voltee verticalmente, esto es necesario porque OpenGL 
	// espera que la imagen tenga su origen en la esquina inferior izquierda, mientras que la mayoría de las imágenes 
	//tienen su origen en la esquina superior izquierda
	unsigned char *image; 
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT); //configuramos las texturas para que se repitan en caso de que las coordenadas de textura sean mayores a 1.0 o menores a 0.0
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT); 
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST_MIPMAP_NEAREST);
	// Diffuse map
	image = stbi_load("images/dados_textura.png", &textureWidth, &textureHeight, &nrChannels, 0); //cargamos la imagen de la textura
	glBindTexture(GL_TEXTURE_2D, texture1); //enlazamos el identificador con el tipo de textura que queremos cargar, en este caso 2D
