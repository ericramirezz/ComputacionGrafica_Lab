...
int FrameIndex = 0;			///en que punto de la linea del tiempo nos encontramos
bool play = false;
int playIndex = 0; //en que punto de la linea del tiempo me encuentro
...
void saveFrame(void)
{
	...
	KeyFrame[FrameIndex].rotFR = rotFR;
	KeyFrame[FrameIndex].rotBL = rotBL;
	KeyFrame[FrameIndex].rotBR = rotBR;
...
void resetElements(void)
{
	...
	rotFR = KeyFrame[0].rotFR;
	rotBL = KeyFrame[0].rotBL;
	rotBR = KeyFrame[0].rotBR;
}
void interpolation(void)
{
	...
	KeyFrame[playIndex].rotFRInc = (KeyFrame[playIndex + 1].rotFR - KeyFrame[playIndex].rotFR) / i_max_steps;
	KeyFrame[playIndex].rotBLInc = (KeyFrame[playIndex + 1].rotBL - KeyFrame[playIndex].rotBL) / i_max_steps;
	KeyFrame[playIndex].rotBRInc = (KeyFrame[playIndex + 1].rotBR - KeyFrame[playIndex].rotBR) / i_max_steps;
}
...
int main()
{
	...
	for (int i = 0; i < MAX_FRAMES; i++)
	{
		...
		KeyFrame[i].rotFR = 0; KeyFrame[i].rotFRInc = 0;
		KeyFrame[i].rotBL = 0; KeyFrame[i].rotBLInc = 0;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::rotate(model, glm::radians(FLegs + rotFL), glm::vec3(-1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		F_LeftLeg.Draw(lightingShader);
		//Front Right Leg
		model = modelTemp;
		model = glm::translate(model, glm::vec3(-0.111f, -0.055f, 0.074f));
		model = glm::rotate(model, glm::radians(FLegs + rotFR), glm::vec3(1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		F_RightLeg.Draw(lightingShader);
		//Back Left Leg
		model = modelTemp;
		model = glm::translate(model, glm::vec3(0.082f, -0.046, -0.218));
		model = glm::rotate(model, glm::radians(RLegs + rotBL), glm::vec3(1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		B_LeftLeg.Draw(lightingShader);
		//Back Right Leg
		model = modelTemp;
		model = glm::translate(model, glm::vec3(-0.083f, -0.057f, -0.231f));
		model = glm::rotate(model, glm::radians(RLegs + rotBR), glm::vec3(-1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
...
void DoMovement()
{
	...
	//cola: V izquierda / B derecha
	if (keys[GLFW_KEY_V]) tail += 1.0f;
	if (keys[GLFW_KEY_B]) tail -= 1.0f;

	//pata delantera izquierda: Q sube / E baja
	if (keys[GLFW_KEY_Q]) rotFL += 1.0f;
	if (keys[GLFW_KEY_E]) rotFL -= 1.0f;

	//pata delantera derecha: R sube / F baja
	if (keys[GLFW_KEY_R]) rotFR -= 1.0f;
	if (keys[GLFW_KEY_F]) rotFR += 1.0f;

	//pata trasera izquierda: Z sube / I baja
	if (keys[GLFW_KEY_Z]) rotBL -= 1.0f;
	if (keys[GLFW_KEY_I]) rotBL += 1.0f;

	// Pata Trasera Derecha: N sube / M baja
...
void Animation() {
	...
	if (play)
	{
		...
		else
		{
			...
			rotFR += KeyFrame[playIndex].rotFRInc;
			rotBL += KeyFrame[playIndex].rotBLInc;
			rotBR += KeyFrame[playIndex].rotBRInc;
